...
bool
Polynomial::FindRoots(const double y, const eigen_utils::Vector& polynomial_in,
...
void
Polynomial::BuildCompanionMatrix(const eigen_utils::Vector& polynomial,
    ...
    do
    {
        ...
        for (int i = 0; i < degree; ++i)
        {
            const double row_norm =
              companion_matrix_offdiagonal.row(i).lpNorm<1>();
            const double col_norm =
              companion_matrix_offdiagonal.col(i).lpNorm<1>();
            ...
            if (exponent != 0)
            {
                    ...
                    gamma * (col_norm + row_norm))
                {
                    ...
                    companion_matrix_offdiagonal.row(i) *=
                      std::ldexp(1.0, -exponent);
                    companion_matrix_offdiagonal.col(i) *=
                      std::ldexp(1.0, exponent);
                }
            }
        }
...
PolynomialFit::PolynomialFit(int _order, eigen_utils::Vector _x,
                             eigen_utils::Vector _y)
  : Polynomial(_order)
{
    poly = new Polynomial(_order);
    samples.clear();
...
void
PolynomialFit::loadSamples(const eigen_utils::Vector& x,
...
PolynomialFit::Fit()
{
  int num_constraints = data_size;
    ...
    eigen_utils::Matrix lhs =
      eigen_utils::Matrix::Zero(num_constraints, num_constraints);
